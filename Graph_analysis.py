...
# Вычисляем смещение времени KSP относительно начала моделирования
...
plt.plot(solve_with_gravity.t, solve_with_gravity.y[0],
         '-r', linewidth=2.5, label="Математическая модель")

# Данные из KSP
plt.plot(time_ksp_filtered, speed_ksp_filtered,
         '-b', linewidth=1.5, label="KSP", alpha=0.8)

plt.xlabel('Время, с', fontsize=14)
plt.ylabel('Скорость, м/с', fontsize=14)
plt.title(f'Сравнение скорости: модель vs KSP', fontsize=16)
